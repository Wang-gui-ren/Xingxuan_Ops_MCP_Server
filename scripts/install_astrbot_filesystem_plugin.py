...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Install the 星璇运维MCP deterministic ops bridge AstrBot plugin into an AstrBot data/plugins directory.",
    )
    parser.add_argument(
        "--astrbot-plugins-dir",
        required=True,
...
def main() -> int:
    args = parse_args()
    source_dir = Path(__file__).resolve().parents[1] / "integrations" / "astrbot_filesystem_command"
    ...
    print(f"Installed 星璇运维MCP deterministic ops bridge plugin to: {target_dir}")
    print("Reload plugins in AstrBot WebUI, then test a deterministic local ops phrase.")
    return 0
